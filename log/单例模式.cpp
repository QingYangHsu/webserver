...
//将构造函数设置为私有，可以防止其他类通过 new 操作符创建单例类的实例。
//这样做可以确保单例类只能通过提供的公共接口（如 GetInstance）获取实例，从而控制实例的数量。
//也就是说，控制了在堆上创建单例类对象分配内存的次数只有一次，就是第一次调用GetInstance的时候