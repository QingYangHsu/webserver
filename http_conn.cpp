...
//开一个全局map表 将数据库kaximoduodb表中的用户名和密码放入map
...
//修改文件描述符 重置epoll上的oneshot事件 ，以确保下一次可读时，epollin能被触发
...
bool http_conn::read() {
    ...
    //读取到的字节
    int bytes_read = 0;
#ifdef connfdLT//水平 因为若本次事件没有处理完 后面还会重复触发
...
//由线程池中的线程主函数worker中的run调用 是处理http请求的入口函数
//这个地方是处理业务逻辑
...
http_conn::HTTP_CODE http_conn::process_read() {
    ...
    || (line_status = parse_line()) ==LINE_OK) {
        ...
        //或者是解析到了一行完整的数据则解析该行的数据 否则若parse_line()返回的是LINE_OPEN,
        //则会跳过整个while循环，直接返回NO_REQUEST,在process_read()上级调用process()中，会重新注册事件，以后继续读入客户端数据

        //获取一行数据，text保存 因为在parse_line()函数中已经将一行的行结束符"\r\n"都替换为'\0'
        text = getline();
        //m_startline更新为下一行的起始位置
        ...
        switch(m_check_state) {
            ...
            case CHECK_STATE_HEADER:
            {
                ...
                break;//2023.12.12：这里是一个问题 为什么这个break会引发错误
            }
...
bool http_conn::process_write(HTTP_CODE ret) {
    switch (ret)
    {
        ...
        case FILE_REQUEST: {//获取文件成功 在process_read中的do_request已经将申请的资源文件通过内存映射映射到一个地址m_file_address
            add_status_line(200, ok_200_title );
            if(m_file_stat.st_size != 0) {
                add_headers(m_file_stat.st_size);
                m_iv[ 0 ].iov_base = writebuff;//响应报文状态行以及头部字段
                m_iv[ 0 ].iov_len = m_write_idx;
                m_iv[ 1 ].iov_base = m_file_address;//内存映射地址，这片区域内容作为响应体
...
bool http_conn::write()
{
    ...
    while(1) {
        ...
        if (bytes_have_send >= m_iv[0].iov_len)//表示已经发送了完整的状态行和头部，还需要发送文件内容部分
        {
            m_iv[0].iov_len = 0;
        ...
        if (bytes_to_send <= 0)//所有数据发送完毕
        {
            // 没有数据要发送了
            unmap();
            modfd(m_epollfd, m_sockfd, EPOLLIN);//再次为该客户端注册可读事件 等待其下一次http申请
...
//解析http请求报文请求行，获得请求方法，目标url，http版本
http_conn::HTTP_CODE http_conn::parse_request_line(char* text) {
    ...
    *m_url++ = '\0';    // 置位空字符，字符串结束符
    ...
    m_version = strpbrk( m_url, " \t" );//m_version指向html后的字符串 即" HTTP/1.1""
    if (!m_version) {
        return BAD_REQUEST;
    }
    ...
    */
    ...
    //当url为/时，将judge.html粘到/屁股后面 即url指向/judge.html   对应用户在浏览器输入http://192.168.88.100:9190/。一定会发送一个get报文服务器会返回judge.html
    if (strlen(m_url) == 1)
...
// 解析HTTP请求的一个头部信息
http_conn::HTTP_CODE http_conn::parse_headers(char* text) {   //text为请求头部的完整一行
    ...
    if( text[0] == '\0' ) {
        ...
        if ( m_content_length != 0 ) {
            ...
            return NO_REQUEST;//返回http请求结果 即继续读取客户数据
        }
...
{
    strcpy(m_real_file, doc_root);//将doc_root拷贝到m_real_file doc_root是本地保存网站各种资源的根目录  m_real_file为"/work/TinyWebServer-raw_version/root"
    ...
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        strncpy(m_real_file + len, m_url_real, FILENAME_LEN - len - 1);//m_real_file为"/work/TinyWebServer-raw_version/root/CGISQL.cgi"
        ...
        if (*(p + 1) == '3')
        {
            ...
            if (map_users.find(name) == map_users.end())//users是一个map表，在该map表中查找有无相同用户名
            {
                ...
                connection_pool::GetInstance()->ReleaseConnection(mysql);//手动归还连接，与781行对应
                m_lock.unlock();
                printf("xqy3\n");
                if (!res)//sql插入语句执行成功
        ...
        {
            if (map_users.find(name) != map_users.end() && map_users[name] == password)
    ...
    if (*(p + 1) == '0')//当http报文为get cgi设置为0 用户直接输入http://192.168.88.100:9190/0，server返回register.html
    {
        ...
        //m_real_file为/work/TinyWebServer-raw_version/root/register.html
