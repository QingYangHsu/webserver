...
class util_timer;   // 前向声明 因为http_conn成员有time的指针类型，很好理解，一个客户对应一个定时器
...
//这一部分定义了一些状态
//client向server的HTTP请求方法，这里只支持GET
enum METHOD {GET = 0, POST, HEAD, PUT, DELETE, TRACE, OPTIONS, CONNECT};

/*
    server解析客户端请求时，主状态机的状态
...
// server解析客户端请求时，从状态机的三种可能状态，即请求部分三个部分每一个部分都对应很多行，定义行的读取状态，分别表示
// 1.读取到一个完整的行 2.行出错 3.行数据尚且不完整
enum LINE_STATUS { LINE_OK = 0, LINE_BAD, LINE_OPEN };

/*
        服务器处理客户HTTP请求的处理结果，报文解析的结果。向客户返回该结果
...
public:
    ...
    //预备动作，server开始运作前先获取表中存储的历史遗留客户
...
//    MYSQL *mysql; //我觉得这个数据成员无意义
    ...
    //处理客户端请求，线程池中八个子线程从任务缓冲队列获得主线程append的客户后，由子线程调用该函数进行逻辑处理
    void process();


    //初始化新接收的客户连接，主要动作是将该客户添加到epollfd监视队列中     
...
private:
    ...
    //客户请求目标文件的文件名以及server向客户返回的请求资源，文件格式都是html
    char * m_url;
    ...
    // 目标文件的状态。通过它我们可以判断文件是否存在、是否为目录、是否可读，并获取文件大小等信息 由系统自定义的默认构造函数来初始化，实际填值在do_request中
    struct stat m_file_stat; 

    // 客户请求的目标文件在do_reauest中被mmap到进程内存中的起始位置
    char* m_file_address;                   

    char *m_string; //存储请求体数据 主要是登录界面用户输入的用户密码，我们会将该记录插入到user表中
